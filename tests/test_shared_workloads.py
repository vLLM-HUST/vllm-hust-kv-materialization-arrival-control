...
def test_paper_tool_workload_alias_resolves_to_shared_catalog_case() -> None:
    case = resolve_case_spec("shared_tool_scaffold_agent")

    assert case["canonical_case_id"] == "shared_scenario_structured_agent_decode"
    assert case["requested_case_id"] == "shared_tool_scaffold_agent"
    assert case["dataset_name"] == "tool-scaffold-agent"
...
def test_generate_case_requests_accepts_custom_tokenizer() -> None:
    ...
    custom_rows = generate_case_requests(
        "dynamic_rag_corpus_update", seed=7, tokenizer=CharTokenizer()
    )
...
def test_generate_public_microbenchmark_case_requests() -> None:
    rows = generate_case_requests(
        "shared_synthetic_shared_prefix_microbenchmark", seed=7
    )
