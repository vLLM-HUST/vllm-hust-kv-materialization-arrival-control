# M1 lifecycle-level 2x2 statistical closure
...
For each workload, round 1, 2, and 3 are three matched lifecycle blocks. Each
block contains the four cells `old/segmented` x `baseline/tuned`. The script
computes each contrast inside a block and then reports the mean paired delta and
a two-sided 95% Student-t confidence interval over the three blocks (df=2):

- seam main effect: mean(segmented cells) - mean(old cells);
- tuning main effect: mean(tuned cells) - mean(baseline cells);
- interaction: segmented tuning effect - old-seam tuning effect;
- endpoint contrast: segmented+tuned - old+baseline.

Relative effects are ratios computed inside each block. Negative TTFT/E2E and
positive throughput changes are favorable. The intervals are descriptive and
unadjusted for multiplicity; with only three blocks they are necessarily wide.
...
All values below are mean paired relative deltas with two-sided 95% CI.
...
| knowledge | interaction | +7.07% [-0.05, +14.19] | +0.75% [+0.12, +1.38] | -0.69% [-1.56, +0.18] |
...
| tool | interaction | -1.31% [-6.74, +4.12] | +0.47% [-4.86, +5.80] | -0.45% [-5.64, +4.75] |
...
This regenerates the per-run and median/IQR artifacts plus:
