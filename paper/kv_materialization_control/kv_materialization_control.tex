...
\texttt{full\_reuse}/\texttt{recompute} plus a block-aligned segmented
\texttt{partial\_reuse} seam with explicit effective-action and fallback
reporting on the current prefix-cache path. The resulting empirical message is
narrow but concrete: the offline study exposes a small yet real
\texttt{partial\_reuse} regime, while live evidence across prefix-rich,
continuation-heavy, and tokenizer-faithful dynamic-RAG cases shows that the
current seam can realize block-aligned \texttt{partial\_reuse}, transition to
\texttt{full\_reuse} when overlap grows, and only supports truthful online
claims when workload construction matches the serving tokenizer.
\end{abstract}
...
\texttt{shared\_scenario\_multi\_turn\_knowledge\_service}, the workload
...
live runtime-boundary taxonomy that truthfully reports the current block-aligned
\texttt{partial\_reuse} seam, demonstrates stable online behavior on
shared-prefix and continuation cases, captures the
\texttt{partial\_reuse}-to-\texttt{full\_reuse} transition on
tokenizer-faithful dynamic RAG, and keeps the remaining gap to exact
token-level materialization explicit.
\end{enumerate}

\section{Limitations and Next Step}
The main limitation remains exact token-level online \texttt{partial\_reuse}
beyond the current block-aligned segmented seam. The next
...
rather than relying on block-aligned segmented reuse plus fallback re-ranking;
