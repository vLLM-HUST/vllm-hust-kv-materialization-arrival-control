...
class FreeKVCacheBlockQueue:
    ...
    def prepend_n(self, blocks: list[KVCacheBlock]) -> None:
        ...
        The front is the next allocation/eviction position. KVPlane uses this
        for denied prefix-cache writes: blocks that were only temporary scratch
        space should be reused before resident prefix-cache blocks.
        """
        if len(blocks) == 0:
            return
