...
class BlockPool:
    ...
    def free_blocks(
        ...
        if not prioritize_uncached_for_reuse:
            self.free_block_queue.append_n(freeable_blocks)
            return

        # Denied KVPlane requests used these blocks as temporary scratch space.
        # Reuse non-cached blocks before evicting resident prefix-cache blocks.
        scratch_blocks = [
            block for block in freeable_blocks if block.block_hash is None
        ]
        cached_blocks = [
            block for block in freeable_blocks if block.block_hash is not None
        ]
        self.free_block_queue.prepend_n(scratch_blocks)
