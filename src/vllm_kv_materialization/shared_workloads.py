...
def recommended_context_window(case_id: str, *, fallback: int | None = None) -> int | None:
...
def _make_prompt(shared_prefix_tokens: int, unique_tokens: int, *, shared_token: str, unique_token: str) -> str:
	parts = [shared_token] * max(shared_prefix_tokens, 1)
	parts.extend([unique_token] * max(unique_tokens, 1))
	return " ".join(parts)


def _generate_public_case_requests(workloads: Any, case_id: str, case: dict[str, Any], *, seed: int) -> list[Any]:
	...
	for index in range(num_prompts):
		...
		shared_prefix_tokens = system_prompt_len + rng.randint(0, max(question_len // 3, 1))
		...
		metadata = {
			...
			"secondary_anchor_ids": [f"{case_id}:turn:{turn_index}", f"{case_id}:band:{group_index % 2}"],
			"primary_anchor_kind": "shared_prefix" if dataset_name == "generated-shared-prefix" else "conversation_cluster",
...
def build_live_workload(
	...
	*,
	...
	effective_concurrency = int(concurrency if concurrency is not None else default_concurrency)
